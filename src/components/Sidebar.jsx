...
export default function Sidebar() {
  const { activeTab, setActiveTab, materials, mobileSidebar, setMobileSidebar, unitSystem, toggleUnit } = useApp();
  const { fc, fy, cover } = materials;

  const displayFc = unitSystem === 'psi' ? `${Math.round(fc * 145.038)} psi` : `${fc} MPa`;
  const displayFy = unitSystem === 'psi' ? `${Math.round(fy * 145.038)} psi` : `${fy} MPa`;
    ...
    <aside className="h-full w-72 bg-slate-900 dark:bg-slate-950 text-white flex flex-col shrink-0">
      ...
      <div className="px-5 py-3 border-b border-slate-700 dark:border-slate-800 bg-slate-800/50">
        <div className="flex items-center justify-between mb-1">
          <p className="text-[10px] uppercase tracking-wider text-slate-500">Materials</p>
          <button
            onClick={toggleUnit}
            className="text-[10px] px-2 py-0.5 rounded bg-slate-700 hover:bg-slate-600 text-slate-300 transition"
          >
            {unitSystem === 'mpa' ? '→ psi' : '→ MPa'}
          </button>
        </div>
        <p className="text-xs font-mono text-emerald-300">f'c = {displayFc}</p>
        <p className="text-xs font-mono text-amber-300">fy = {displayFy}</p>
        <p className="text-xs font-mono text-slate-400">Cover = {cover} mm</p>
      </div>
